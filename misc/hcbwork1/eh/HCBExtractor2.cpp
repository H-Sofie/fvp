...
int main(int argc, char *argv[]) {
	...
	if (argc >= 3) oriFileName=argv[1], oriResultFileName=argv[2];
	if (argc == 4) global_offset = atoi(argv[3]);
...
//	fp2 = fopen(cnResultFileName.c_str(), "w");
	...
	while (pos < lenOri) {
		...
		switch(c) {
			...
			case 0x06:
				nextpos = pos + 1; isJmp = true; break;
			default:
				nextpos = pos + 1; break;
		}
		if (isPushStr) {
			int len = binOri[nextpos];
			// if (pos >= 0x8d818) { fprintf(lout, "len = %02X\n", len); }
			fprintf(fp, "{0x%08X}\n", pos);
...
//			SKIPCN:
		...
		if (isJmp) {
			int pos2 = getPos2(nextpos);
			if (pos2 >= lenOri) { nextpos = nextpos + 4; }
			else {
				uchar c2 = binOri[pos2];
				if (c2 != 0x0E) {
					nextpos = nextpos + 4;
				} else {
					int len2 = binOri[pos2+1];
					if (binOri[pos2 + 2 + len2] != 0x06) {
						nextpos = nextpos + 4;
					} else {
						int pos3 = getPos2(pos2 + 2 + len2 + 1);
						if (pos3 >= lenOri || (pos3 - pos >= 0xFF)) { nextpos = nextpos + 4; }
						else {
							int cntpos = pos2 + 2;
							fprintf(fp, "{0x%08X}\n", pos);
							for (int i = 0; i < len2-1; i++) {
